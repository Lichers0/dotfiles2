...
| `--model` | string? | Модель для агента. Без значения: показать таблицу моделей |
...
# Показать таблицу доступных моделей
aiwr --model

# Запуск с конкретной моделью
aiwr "fix bug" --agent claude --model sonnet

# Модель с дополнительными аргументами (CLI переопределяет)
aiwr "fix bug" --model opus -- --max-tokens 4000
```
...
Модели хранятся в `src/aiwr/models.py`:

```python
# src/aiwr/models.py
MODELS = {
    "agent_name": [
        {"alias": "short", "default": True, "model_id": "full-model-id", "extra_args": []},
    ],
}
```
...
| Alias | Default | Model ID | Extra args |
|-------|---------|----------|------------|
| `opus` | ✓ | `opus` | — |
| `sonnet` | | `sonnet` | — |
| `haiku` | | `haiku` | — |
...
1. Определяется агент (`--agent` или default `claude`)
2. Резолвится модель для этого агента:
   - Если `--model alias` указан → ищем alias у агента
   - Если `--model` не указан → берём модель с `default: True`
3. При ошибке: `"Model 'gpt52' not found for agent 'claude'. Available: opus, sonnet, haiku"`

### Приоритет аргументов

1. `extra_args` из models.py (базовые для модели)
2. `extra_args` из командной строки (после `--`) — **переопределяют**

```bash
# models.py: opus.extra_args = ["--max-tokens", "8000"]
# CLI:
aiwr "hi" --model opus -- --max-tokens 4000
# Результат: --max-tokens 4000 (CLI победил)
```
...
## 9.1. Продолжение сессии (--session)
...
└── src/
    └── aiwr/
        ...
        ├── models.py           # Конфигурация моделей для агентов
        ├── agents/
...
| Неизвестная модель | Ошибка: "Model '{alias}' not found for agent '{agent}'. Available: ..." |
...
## 18. Будущие расширения (не в MVP)
