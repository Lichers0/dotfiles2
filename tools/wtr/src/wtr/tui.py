...
class CreateWorktreeDialog(ModalScreen[tuple[str, str] | None]):
    """Dialog for creating new worktree."""
    ...
    CreateWorktreeDialog #base-list {
        height: 8;
    ...
    def __init__(self, branch_name: str, default_base: str, branches: list[str], is_new_branch: bool):
        super().__init__()
    ...
    def compose(self) -> ComposeResult:
        title = "Create new branch + worktree" if self.is_new_branch else "Create worktree"
        with Container():
            yield Label(title, classes="title")
            yield Label(f"Branch: {self.branch_name}")
            yield Label("Base branch:", classes="field-label")
            yield Input(value=self.default_base, id="base-branch")
            yield ListView(id="base-list")
            with Horizontal():
                yield Button("Create", id="create", variant="primary")
                yield Button("Cancel", id="cancel")

    def on_mount(self) -> None:
        self._refresh_base_list(self.default_base)
        self.query_one("#base-branch", Input).focus()

    def _refresh_base_list(self, filter_text: str = "") -> None:
        """Refresh base branch list with fuzzy filtering."""
        list_view = self.query_one("#base-list", ListView)
        list_view.clear()
        self._filtered_branches = fuzzy_match(self.branches, filter_text)
        for branch in self._filtered_branches:
            list_view.append(ListItem(Label(branch)))

    def on_input_changed(self, event: Input.Changed) -> None:
        if event.input.id == "base-branch":
            self._refresh_base_list(event.value)

    def on_list_view_selected(self, event: ListView.Selected) -> None:
        list_view = self.query_one("#base-list", ListView)
        if list_view.index is not None and self._filtered_branches:
            branch = self._filtered_branches[list_view.index]
            self.dismiss((self.branch_name, branch))

    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "create":
            base_input = self.query_one("#base-branch", Input)
            self.dismiss((self.branch_name, base_input.value))
        else:
            self.dismiss(None)

    def on_input_submitted(self, event: Input.Submitted) -> None:
        self.dismiss((self.branch_name, event.value))
...
class WorktreeApp(App[Path | None]):
    ...
    def _on_create_dialog(self, result: tuple[str, str] | None) -> None:
    ...
    def _on_goto_confirm(self, go: bool, path: Path) -> None:
        ...
        if go:
            # Update symlink to this worktree
